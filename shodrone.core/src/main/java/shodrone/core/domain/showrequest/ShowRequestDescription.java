...
@Embeddable
public class ShowRequestDescription implements ValueObject {
    @XmlElement
    @JsonProperty
    private Blob descriptionDocument;


    public ShowRequestDescription() {
    }

    public ShowRequestDescription(File descriptionFile) {
        validate(descriptionFile);

        this.descriptionDocument = fileToBlob(descriptionFile);
    }

    public ShowRequestDescription(Blob descriptionDocument) {
