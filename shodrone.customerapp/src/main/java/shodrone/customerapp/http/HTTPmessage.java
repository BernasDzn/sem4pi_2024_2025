package shodrone.customerapp.http;
...
public class HTTPmessage {
    ...
    /**
     * Reads the header line from the input stream until a CR (Carriage Return) is found.
     * The line is returned as a String, without the CR and LF characters.
     * @param in
     * @return
     * @throws IOException
     */
    static private String readHeaderLine(DataInputStream in) throws IOException {
        ...
        do { // Reads the input stream until a CR (Carriage Return) is found
            readVal = in.read();
            if( readVal == -1 ){
                throw new IOException("End of stream reached while reading header line (expected CR)");
            }
            if( readVal != CR ){
                headString += (char) readVal;
            }
        } while( readVal != CR);

        readVal = in.read(); // reads the LF (Line Feed) character
        if(readVal == -1){
            throw new IOException("End of stream reached while reading header line (expected LF)");
        }
        return headString;
    }
    
    /**
     * Writes a header line to the output stream, followed by a CR (Carriage Return) and LF (Line Feed).
     * @param out
     * @param line
     * @throws IOException
     */
        ...
        do {
            ...
            if( headerLine.toUpperCase().startsWith(CONTENT_TYPE.toUpperCase()) ) {
                ...
                try { 
                    ...
                    throw new IOException("Invalid Content-Length: " + cLen);
                }
